...
def generer_meteo(ville: str) -> tuple[str, str] | tuple[None, None]:
    try:
        ...
        temperatures = [t if t is not None else 15.0 for t in res['hourly']['temperature_2m'][idx_depart:fin]]
        ...
        # Bloc inférieur (Courbe de température & Barres de pluie)
        ...
        l1 = ax1.plot(x, temperatures, color='#ff4d4d', linewidth=2.5, label='Température (°C)', marker='o', markersize=5)
        ...
        ax1.set_title(f"BULLETIN METEO — {nom_complet.upper()}", fontsize=12, fontweight='bold', pad=15, color='#2c3e50', loc='left')
        fig.tight_layout()
        
        chemin_image = f"meteo_{ville}.png"
        plt.savefig(chemin_image, bbox_inches='tight')
        plt.close()

        max_temp = max(temperatures)
        min_temp = min(temperatures)
        global_rain = "Risque de pluie à prévoir 🌧️" if max(probas_pluie) > 30 else "Rencontre avec le soleil ou temps sec ☀️"
        
        texte_telegram = (
            f"📊 METEO DU JOUR - {nom_complet}\n\n"
            f"🌡️ Températures : Min {round(min_temp,1)}°C / Max {round(max_temp,1)}°C\n"
            f"☁️ Tendance : {global_rain}\n\n"
            f"Découvre ton graphique horaire ci-dessous !"
        )
        
        return texte_telegram, chemin_image
        
    except Exception as e:
        logging.error(f"Erreur lors de la génération météo: {str(e)}")
        return None, None

async def commande_meteo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("❌ Donne-moi une ville ! Exemple : `/meteo Paris`", parse_mode="Markdown")
        return

    ville_cible = " ".join(context.args)
    message_attente = await update.message.reply_text(f"⏳ Récupération de la météo pour *{ville_cible}*...", parse_mode="Markdown")
    
    texte, chemin_image = generer_meteo(ville_cible)
    
    if texte and chemin_image:
        with open(chemin_image, 'rb') as photo:
            await update.message.reply_photo(photo=photo, caption=texte)
        
        os.remove(chemin_image)
        await message_attente.delete()
    else:
        await message_attente.edit_text(f"❌ Impossible de trouver la météo pour la ville : *{ville_cible}*.", parse_mode="Markdown")

if __name__ == "__main__":
    TOKEN = os.environ.get("TELEGRAM_TOKEN")
    
    if not TOKEN:
        # C'est ici que le message d'erreur a été corrigé avec le bon nom de fichier
        print("❌ Erreur : La variable d'environnement TELEGRAM_TOKEN est manquante.")
        exit(1)
        
    app = Application.builder().token(TOKEN).build()
    app.add_handler(CommandHandler("meteo", commande_meteo))
    
    print("🚀 Bot météo démarré et prêt à recevoir des commandes !")
    app.run_polling()
